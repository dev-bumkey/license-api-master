...
*/
package run.acloud.api.k8sextended.util;
...
public class ModelMapper {
   ...
   * io.kubernetes.client.util.ModelMapper#DEFAULT_DISCOVERY_REFRESH_INTERVAL}.
   *
  ...
  private static void initModelMap() throws IOException {
    ...
    // Package Name : run.acloud.api.k8sextended.models
    String pkgNameExtended = NetworkingV1beta1Ingress.class.getPackage().getName();
    // Package Name : BOOT-INF.classes.run.acloud.api.k8sextended.models
    String pkgNameExtendedWithPrefix = String.format("BOOT-INF.classes.%s", NetworkingV1beta1Ingress.class.getPackage().getName());
    ClassPath cpExtended = ClassPath.from(Yaml.class.getClassLoader());
    Set<ClassPath.ClassInfo> allClassesExtended = cpExtended.getTopLevelClassesRecursive(pkgNameExtendedWithPrefix);

    for (ClassPath.ClassInfo classInfo : allClassesExtended) {
//      Class clazz = classInfo.load();
      Class<?> clazz = loadClass(String.format("%s.%s", pkgNameExtended, classInfo.getSimpleName()), Yaml.class.getClassLoader());
      if (clazz == null) {
        continue;
      }
    ...
    for (String classInfo :
            getClassNamesFromPackage(
                    io.kubernetes.client.util.Yaml.class.getClassLoader(), "io.kubernetes.client.openapi.models")) {
      Class<?> clazz = loadClass(classInfo, io.kubernetes.client.util.Yaml.class.getClassLoader());
      if (!KubernetesObject.class.isAssignableFrom(clazz)
              && !KubernetesListObject.class.isAssignableFrom(clazz)) {
        continue;
      }
  ...
  private static Pair<String, String> getApiGroup(String name) {
    return preBuiltApiGroups.entrySet().stream()
            ...
            .findFirst()
            .orElseGet(() ->new MutablePair(null, name));
  }
