package run.acloud.api.pl.vo;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.Setter;
import run.acloud.api.build.vo.BuildRunVO;
import run.acloud.api.pl.enums.PlStatus;
import run.acloud.commons.vo.HasUpserterVO;

import java.util.Date;
import java.util.List;

@Getter
@Setter
@EqualsAndHashCode(callSuper = false)
@Schema(description = "파이프라인 실행 빌드 모델")
public class PlRunBuildVO extends HasUpserterVO {

    @Schema(title = "파이프라인 실행 빌드 번호")
    private Integer plRunBuildSeq;

    @Schema(title = "파이프라인 실행 번호")
    private Integer plRunSeq;

    @Schema(title = "파이프라인 리소스 빌드 번호")
    private Integer plResBuildSeq;

    @Schema(title = "빌드 번호")
    private Integer buildSeq;

    @Schema(title = "빌드 이전 실행 번호")
    private Integer buildPrevRunSeq;

    @Schema(title = "빌드 실행 번호")
    private Integer buildRunSeq;

    @Schema(title = "이미지 URL")
    private String imgUrl;

    @Schema(title = "빌드 태그")
    private String buildTag;

    @Schema(title = "빌드 단계 컨텐츠 - 별도 빌드 태그 생성시 셋팅한 build step 정보들, json string 데이터", accessMode = Schema.AccessMode.READ_ONLY)
    private String buildCont;

    @Schema(title = "빌드 설정 정보, buildCont 문자열을 VO로 변환한 필드, 기존 실행했던 BuildRun 정보나 이후 수정 및 추가한 빌드 설정 정보")
    private BuildRunVO buildConfig;

    @Schema(title = "실행 순서")
    private Integer runOrder;

    @Schema(title = "실행 여부")
    private String runYn = "N";

    @Schema(title = "실행 상태 - WAIT, RUNNING, FAILED, DONE")
    private PlStatus runStatus;

    @Schema(title = "실행 로그")
    private String runLog;

    @Schema(title = "시작 시간")
    private Date beginTime;

    @Schema(title = "종료 시간")
    private Date endTime;

    @Schema(title = "작업시간(단위:초) : endTime - beginTime")
    private Integer runTimeBySec;

    @Schema(title = "buildRun 정보를 찾지 못했을 경우 셋팅, (true : 존재, false : 미존재)")
    private boolean hasBuildRun;

    @Schema(title = "Build-워크로드 리소스 간의 맵핑 정보")
    private List<PlRunBuildDeployMappingVO> runBuildDeployMapping;

}