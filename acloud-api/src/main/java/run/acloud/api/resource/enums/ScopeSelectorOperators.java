...
public enum ScopeSelectorOperators implements EnumCode {
	...
	ScopeSelectorOperators(String value, SelectorOperatorsValidType validType){
		this.value = value;
	...
	public static List<Map<String, String>> getValueList(){
		return Arrays.asList(ScopeSelectorOperators.values()).stream().map(s -> s.toMap()).collect(Collectors.toList());
	}
