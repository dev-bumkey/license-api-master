...
public class AuditLogger {
    ...
    public Map<String, Object> makeRequestDump(HttpServletRequest request) throws IOException {
        ...
        Map<String, Object> uriTemplateVariables = (Map<String, Object>) request.getAttribute(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE);
...
//            Thread.sleep(3000);
             ...
             * ** JsonString 형태로 받아 Object로 변환시 캐스팅 오류 발생으로 주석 처리..
              */
//            ResultVO result = JsonUtils.fromGson(resultStr, ResultVO.class);
...
//            aLog.debug("===== : method : " + method);
//            aLog.debug("===== : className : " + className);
//            aLog.debug("===== : methodName : " + methodName);
//            aLog.debug("===== : userSeq : " + userSeq);
//            aLog.debug("===== : userServiceSeq : " + userServiceSeq);
//            aLog.debug("===== : userRole : " + userRole);
//            aLog.debug("===== : duration : " + duration);
//            aLog.debug("===== : requestData : " + JsonUtils.toGson(requestData));
            ...
            auditLog.setAuditLogCode(className+"."+methodName);
            ...
            if(resultResult != null) { // ResultVO를 Response Data에 포함시킬 것인가.. (Response Status는 별도 필드로 표시중이므로 제외함..)
                if(methodName.equalsIgnoreCase("login")
                        ...
                        || methodName.equalsIgnoreCase("loginPlatformUser")
                        || methodName.equalsIgnoreCase("logout")
                        || "OpenapiController.getApiToken".equalsIgnoreCase(auditLog.getAuditLogCode())
                ) {
            ...
            if (StringUtils.equalsAnyIgnoreCase(auditLog.getAuditLogCode(), "TerminalController.addLogTerminalOnOpen")) {
            ...
            /** insert Audit Log */
            int count = auditService.addAuditLog(auditLog);
...
//                Thread.sleep(3000);
                ...
                switch (auditAdditionalTypeOptional.get()) {
                    case LAUNCH_TEMPLATE:
                        ...
                        if(servicemap != null) {
                            ...
                            auditLog.setResourceData(JsonUtils.toPrettyString(auditResourceDatas));
                            auditLog.setAuditLogSeq(null);
                            auditLog.setAuditLogCode(className+"."+methodName+"addServicemap");
                            count = auditService.addAuditLog(auditLog);
     ...
     * @throws Exception
     */
    private void writeAdditionalResourceLog(String resourceName, String additionalLogCode, Map<String, Object> auditResourceDatas, AuditLogVO auditLog) throws Exception {
        if(StringUtils.isNotBlank(resourceName)) {
            auditResourceDatas.put("resourceName", resourceName);
            auditLog.setResourceName(resourceName);
            auditLog.setResourceData(JsonUtils.toPrettyString(auditResourceDatas));
            auditLog.setAuditLogSeq(null);
            auditLog.setAuditLogCode(auditLog.getControllerName() + "." + auditLog.getMethodName() + additionalLogCode);
            int count = auditService.addAuditLog(auditLog);
            aLog.debug("### Additional Audit Log (" + auditLog.getAuditLogCode() + ") Write (" + count + ") : " + auditLog.getUri());
        }
        else {
            aLog.debug("### Additional Audit Log (" + auditLog.getAuditLogCode() + ") Write Failure (name is null) : " + auditLog.getUri());
        }
    }
...
//            Thread.sleep(6000);
            ...
            if(isCatalogue) {
                auditLog.setAuditLogCode(className + "." + methodName + "addWorkload");
            }
            else {
                auditLog.setAuditLogCode(className + "." + methodName + "Process");
            }
            auditLog.setHttpMethod(method);
            auditLog.setUri(requestUri);
            auditLog.setControllerName(className);
...
//            Map<String, Object> uriTemplateVariables = (Map) auditProcessingDatas.get(CommonConstants.AUDIT_REQUEST_DATA_PATH);
            ...
            auditLog.setAuditLogCode(className + "." + methodName + "installPackage");
        ...
        try {
            ...
            switch (className) {
                ...
                case "InternalOtpController": { // InternalOtpController에 대한 처리 (사용자 전체 OTP 정보)
                    ...
                    if ("AccountController.addAccount".equals(auditLogCode)) { // Account 생성
                        ...
                        if ("AccountController.addUserOfAccount".equals(auditLogCode) ||
                            ...
                            if(user == null &&
                                "AccountController.removeUserOfAccount".equals(auditLogCode) &&
                                "ok".equals(resultStatus) && resultResult != null) {
                                user = (UserVO) resultResult;
                            }
                ...
                case "ServiceController": { // ServiceController에 대한 처리. (워크스페이스)
                    ...
                    else if ("updateServicemapOfService".equals(methodName) || "renameServicemapOfService".equals(methodName) || "removeServicemapOfService".equals(methodName)) {
                        relationKey = getIntegerSeq(uriTemplateVariables, "servicemapSeq");
                        relationKeyName = "servicemapSeq";
                        ServicemapVO servicemap = servicemapService.getServicemap(relationKey, null);
                        auditResourceDatas.put("resourceName", Optional.ofNullable(servicemap).map(ServicemapVO::getServicemapName));
                    }
                    else if ("udpateNamespaceOfServicemapByYaml".equals(methodName)) {
                        K8sDeployYamlVO reqBodyObj = JsonUtils.fromJackson(requestBody, K8sDeployYamlVO.class);
                        relationKey = this.setRelationAndGetServicemapSeq(auditResourceDatas, uriTemplateVariables);
                        auditResourceDatas.put("resourceName", uriTemplateVariables.get("namespaceName"));
                        if(reqBodyObj != null) {
                            auditResourceDatas.put("yaml", reqBodyObj.getYaml());
                        }
                    }
                    else if ("addServicemapOfService".equals(methodName)) { // 워크스페이스에 서비스맵 추가
                        relationKey = getIntegerSeq(uriTemplateVariables, "serviceSeq");
                        relationKeyName = "serviceSeq";
                ...
                case "ServicemapController": { // ServicemapController에 대한 처리. (서비스맵)
                    ...
                    if ("addServicemapV2".equals(methodName)) {
                        if ("ok".equals(resultStatus) && resultResult != null) {
                            ...
                            // Serviccemap 생성 실패시 Audit Logging...
                            ServicemapAddVO servicemap = null;
                            if (StringUtils.isNotBlank(requestBody) && requestBody.startsWith("{")) {
                                servicemap = JsonUtils.fromJackson(requestBody, ServicemapAddVO.class);
                            }
                    ...
                    else if ("udpateNamespaceByYaml".equals(methodName)) {
                        K8sDeployYamlVO reqBodyObj = JsonUtils.fromJackson(requestBody, K8sDeployYamlVO.class);
                        relationKey = this.setRelationAndGetServicemapSeq(auditResourceDatas, uriTemplateVariables);
                        auditResourceDatas.put("resourceName", uriTemplateVariables.get("namespaceName"));
                        if(reqBodyObj != null) {
...
//                        relationKey = groupService.getGroupWithComponentSeq(this.getIntegerSeq(uriTemplateVariables, "componentSeq")).getGroupSeq();
                ...
                case "PipelineFlowController": { // PipelineFlowController에 대한 처리. (파이프라인)
                    // 파이프라인 플로우 생성 / 수정
                ...
                case "PipelineBuildRunController": { // PipelineBuildRunController에 대한 처리. (빌드)
                    // 빌드 실행 / 빌드 이력에서 빌드 실행
...
//                    if ("cancelBuildRun".equals(methodName)) {
                ...
                case "TemplateController": { // TemplateController에 대한 처리. (카탈로그)
                    /** API 유형별 로깅 **/
...
//                                ServerGuiVO serverGui = JsonUtils.fromJackson(requestBody, ServerGuiVO.class);
                        ...
                        if ("ok".equals(resultStatus) && resultResult != null) {
                            auditResourceDatas.put("resourceName", resultResult);
                        }
                    }
            ...
            if (relationKey != null && StringUtils.isNotBlank(relationKeyName)) { // serviceRelation 조회가 가능 = 정상케이스 => 리소스 정보 입력.
                ...
                switch (className) {
                    case "ServiceController" : {
                        if (StringUtils.equals("servicemapSeq", relationKeyName)) { // 워크스페이스 권한의 사용자가 서비스맵 APi 사용 케이스
     ...
     * @return
     */
    private AuditLogVO setServiceRelation(AuditLogVO auditLog, ServiceRelationVO serviceRelation) {
    ...
    /**
     * 기존 service map를 URI기반으로 사용하는 API를 Cluster, Namespace기반 API로 변경하면서 이를 체크하고 서비스 릴레이션 정보를 기록하기 위함.. 2019.11.29
     * @param auditResourceDatas
